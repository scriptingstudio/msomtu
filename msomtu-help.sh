if [[ "${LANG%\.*}" == "ru_RU" ]]; then
	...
	printb "ПРИМЕЧАНИЯ:"
	print-column 0 $p6 "" "В скрипте реализован принцип безопасного исполнения - \"защита от дураков\". Режим исполнения по умолчанию - эмуляция. Скрипт не сможет внести изменения в систему без параметра '-run'." '-'
	...
	print-column 0 $p6 "" "Если удаляете шрифты, также удаляйте файлы списков шрифтов. Папка 'DFonts' и файлы списков шрифтов не обязательны - их можно удалять. Шрифты в папках 'DFonts' и 'Fonts' не видны другим приложениям. Некоторые некириллические шрифты могут быть полезны - сохраните их перед удалением." '-'
	...
	print-column 0 $p6 "" "Классификация шрифтов в предопределенных наборах (по убыванию): кириллические, некириллические, иероглифические, символьные, системные. Предопределенные шрифтовые наборы не пересекаются." '-'
	echo
	fi
...
#	local fmt21='%*s %-40s  %s %b\n'
	...
	print-column $p4 $p20 "lang_list" "Список языков локализации: ru pl de и т.д.; смотрите имена файлов с параметром '-verbose'. Значение по умолчанию: 'en ru'." ":"
	print-column $p4 $p20 "proof_list" "Список языков проверки правописания: russian finnish german и т.д.; смотрите имена файлов с параметром '-verbose'. Можно использовать символ подстановки '*'. Значение по умолчанию: 'english russian'." ":"
	...
	print-column $p4 $p20 "-verbose" "Ключ. Показывает детальную информацию по объектам выбора в режиме эмуляции. С аргументом 'nl' отменяет вывод списка файлов. Не зависит от '-run'." ":"
