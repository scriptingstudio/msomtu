...
NOTES=(
	...
	"Если удаляете шрифты, также удаляйте файлы списков шрифтов (font*.plist); см. раздел ПАРАМЕТРЫ. Папка 'DFonts' и файлы списков шрифтов не обязательны - их можно удалять. Шрифты в папках 'DFonts' и 'Fonts' не видны другим приложениям. Некоторые некириллические шрифты могут быть полезны - сохраните их перед удалением."
...
PARAMETERS=(
	...
	"-help||Ключ. Показывает страницу помощи. Язык текста зависит от системных языковых настроек. Есть два вида страницы: краткая и полная. По умолчанию (без параметров) выводится краткая страница. Для полного вида используйте параметр '-help full'. С аргументом 'en' выводит английскую страницу помощи."
